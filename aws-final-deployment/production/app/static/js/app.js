...
class CertificateApp {
    ...
    showLoading(show) {
        ...
        if (show) {
            ...
            if (!submitBtn) { console.error("❌ submitBtn not found in DOM"); return; } submitBtn.disabled = true;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("✅ CertificateApp initialized globally");

    // Cache submit button after DOM ready
    const submitBtn = document.getElementById("submitBtn");
    // Manual form binding
    const form = document.getElementById("certificateForm");
    if (form) {
        ...
        console.error("❌ certificateForm not found – cannot bind listener");
    }
});
